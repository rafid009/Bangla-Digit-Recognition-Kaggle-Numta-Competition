...
def deresolution(img):
	scale_factor = np.random.uniform(0.3, 0.6) # percent of original size
	width = int(img.shape[1] * scale_factor)
	height = int(img.shape[0] * scale_factor)
	dim = (width, height)
	downscaled = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)

	width = int(img.shape[1] /scale_factor)
	height = int(img.shape[0] / scale_factor)
	dim = (width, height)
	upscaled = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)
...
function_list = [rotate_image, sp_noise, erase, blur, shear, border, superimpose, shift, zoom, deresolution]
function_prob = [0.20,         0.15,     0.10,  0.10, 0.10,  0.03,   0.10,        0.05,  0.10, 0.07]
